...
export function doAutoCompletion(documents: TextDocuments<TextDocument>, neo4j: Neo4jSchemaPoller) {
  return (completionParams: CompletionParams) => {
    ...
    const yieldTriggered = shouldAutoCompleteYield(textDocument.getText(), offset);
    const manualOrCharacterOrInwordTriggered = completionParams.context?.triggerCharacter !== ' ';
