...
import { ConnectionItem, connectionTreeDataProvider } from './treeviews/connectionTreeDataProvider';
...
export function registerDisposables(): Disposable[] {
  ...
  disposables.push(
    commands.registerCommand(CONSTANTS.COMMANDS.SWITCH_LINTER_COMMAND, manuallyAdjustLinter),
    linterStatusBarItem,
  );

  disposables.push(
    window.registerWebviewViewProvider('neo4jQueryDetails', queryDetailsProvider, {
      webviewOptions: { retainContextWhenHidden: true },
    }),
    window.registerWebviewViewProvider('neo4jQueryVisualization', queryVisualizationProvider, {
      webviewOptions: { retainContextWhenHidden: true },
    }),
    window.registerTreeDataProvider('neo4jConnections', connectionTreeDataProvider),
    window.registerTreeDataProvider(
      'neo4jDatabaseInformation',
      databaseInformationTreeDataProvider,
    ),
    window.registerTreeDataProvider('neo4jParameters', parametersTreeDataProvider),
    ...
    commands.registerCommand(CONSTANTS.COMMANDS.CREATE_CONNECTION_COMMAND, createConnectionPanel),
    commands.registerCommand(
      CONSTANTS.COMMANDS.DELETE_CONNECTION_COMMAND,
      promptUserToDeleteConnectionAndDisplayConnectionResult,
    ),
    commands.registerCommand(CONSTANTS.COMMANDS.CONNECT_COMMAND, (connectionItem: ConnectionItem) =>
      toggleConnectionItemsConnectionState(connectionItem),
    ),
    commands.registerCommand(
      CONSTANTS.COMMANDS.DISCONNECT_COMMAND,
      (connectionItem: ConnectionItem) => toggleConnectionItemsConnectionState(connectionItem),
    ),
    commands.registerCommand(CONSTANTS.COMMANDS.REFRESH_CONNECTIONS_COMMAND, () => {
    ...
    commands.registerCommand(CONSTANTS.COMMANDS.INTERNAL.SWITCH_DATABASE, switchToDatabaseWithName),
    commands.registerCommand(
      CONSTANTS.COMMANDS.CYPHER_FILE_FROM_SELECTION,
      cypherFileFromSelection,
    ),
    commands.registerCommand(CONSTANTS.COMMANDS.ADD_PARAMETER, addParameter),
    commands.registerCommand(CONSTANTS.COMMANDS.DELETE_PARAMETER, removeParameter),
    commands.registerCommand(CONSTANTS.COMMANDS.EDIT_PARAMETER, editParameter),
    commands.registerCommand(CONSTANTS.COMMANDS.CLEAR_PARAMETERS, clearAllParameters),
    commands.registerCommand(CONSTANTS.COMMANDS.INTERNAL.EVAL_PARAMETER, evaluateParam),
    commands.registerCommand(
      CONSTANTS.COMMANDS.INTERNAL.FORCE_DELETE_PARAMETER,
      removeParameterByKey,
    ),
    commands.registerCommand(CONSTANTS.COMMANDS.INTERNAL.FORCE_DISCONNECT, forceDisconnect),
    commands.registerCommand(CONSTANTS.COMMANDS.INTERNAL.FORCE_CONNECT, forceConnect),
  );
