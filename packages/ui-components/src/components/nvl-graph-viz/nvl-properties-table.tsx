...
import { ClickableUrls } from '../shared/clickable-urls';
    ...
    <>
      ...
      {valueIsTrimmed && (
        <button
          ...
          className="text-palette-primary-text"
        >
          {' Show all'}
        </button>
      )}
...
export const PropertiesTableNvl = ({
  propertiesWithTypes,
  paneWidth,
}: PropertiesTableNvlProps): JSX.Element => {
  return (
    <div
    ...
    >
      ...
      {propertiesWithTypes.map(({ key, type, value }) => {
        return (
          <div
            key={key}
            title={type}
          ...
          >
            <Typography
              variant="body-medium"
              className="line-clamp-3 basis-1/3 font-black"
            >
              {key}
            </Typography>
