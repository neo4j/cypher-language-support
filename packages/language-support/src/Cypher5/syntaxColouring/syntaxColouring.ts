...
import { getTokenPosition, tokenPositionToString } from '../../helpers';
import { BracketType, CypherTokenType, ParsedCypherToken } from '../../types';
    ...
    if (shouldAssignTokenType(token)) {
      ...
      if (!tokenFinished) {
        tokens.slice(i + 1, tokens.length).forEach((nextToken) => {
          if (nextToken.type !== CypherCmdLexer.EOF) {
