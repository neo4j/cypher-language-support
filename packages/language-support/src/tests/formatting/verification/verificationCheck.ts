...
function errorString(message: string, query: string, formatted: string): string {
  return `${message},
---------   QUERY BEFORE START  ------------
${query}
---------   QUERY BEFORE END    ----------
...
function verifyFormatting(query: string): void {
  ...
  if (originalNonWhitespaceCount !== formattedNonWhitespaceCount) {
    throw new Error(errorString("Non-whitespace character count mismatch", query, formatted));
  }
  // AST integrity check
  if (formattedStandardized !== queryStandardized) {
    throw new Error(errorString("Standardized query does not match standardized formatted query", query, formatted));
  }
  // Idempotency check
  const formattedTwice = formatQuery(formatted);
  if (formattedTwice !== formatted) {
    throw new Error(errorString("Formatting is not idempotent", query, formatted));
  }
}
