...
describe('Misc auto-completion', () => {
  ...
  test('Should not slow down completions on query with many WITH-clauses', () => {
    const query = `
      MATCH (n1) WITH * LIMIT 1
  MATCH (n2) WITH * LIMIT 1
  MATCH (n3) WITH * LIMIT 1
  ...
  test('Should not slow down completions on query with many WITH-clauses, also when caret is not at the end', () => {
 ...
 `
  ...
  test('Should quickly complete query with many MATCH clauses', () => {
    const query = `
      MATCH (n1) LIMIT 1
  MATCH (n2) LIMIT 1
  MATCH (n3) LIMIT 1
